...
@section('title', 'Register as Team')
...
<style>
    ...
    .input-prefix { display: flex; align-items: center; border: 1px solid #eee; border-radius: 10px; overflow: hidden; }
    .input-prefix span { padding: 10px 12px; background: #f8f8f8; font-size: 13px; color: #888; border-right: 1px solid #eee; white-space: nowrap; }
...
<div class="auth-page">
    <div class="auth-card">
        ...
        <div class="badge-wrap">
            <span class="badge-type"><i class="fas fa-users"></i> Team Account</span>
        </div>

        <h2 class="auth-title">Create your account</h2>
        <p class="auth-subtitle">Register your team and start collaborating</p>
        ...
        <form method="POST" action="{{ route('front.register.team.post') }}">
            @csrf

            <div class="form-group">
                <label>Team Name</label>
                <input type="text" name="team_name" value="{{ old('team_name') }}" placeholder="Your team name" required>
            </div>

            <div class="form-group">
                <label>Email</label>
                <input type="email" name="email" value="{{ old('email') }}" placeholder="team@email.com" required>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="input-prefix">
                    <span>₪ / hr</span>
                    <input type="number" name="hourly_rate" value="{{ old('hourly_rate') }}" placeholder="e.g. 50" min="1" required>
                </div>
            </div>

            <div class="form-group">
                <label>Bio <span style="color:#aaa;font-weight:400;">(optional)</span></label>
                <textarea name="bio" placeholder="Describe your team's expertise and services...">{{ old('bio') }}</textarea>
            </div>
            ...
            <button type="submit" class="submit-btn">Create Team Account</button>
        </form>
